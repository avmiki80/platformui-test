...
export class RolesGuard implements CanActivate {
  ...
  canActivate(
    ...
    if (this.authService.isLoggedIn) { // если пользователь уже залогинен
      ...
      // return this.userHasRequiredRole(this.authService.currentUser.getValue().userRoleSet, next.data.allowedRoles);
      return this.userHasRequiredRole(this.authService.currentUser.getValue().roleSet, next.data.allowedRoles);
    }
    ...
    return this.authService.autoLogin().pipe( // не путать pipe из angular и pipe из rxjs
      map(result => {
        if (result) {
          ...
          // если у пользователя есть права на эту страницу - вернется true и произойдет переход на запрошенный url
          // return this.userHasRequiredRole(user.userRoleSet, next.data.allowedRoles);
          return this.userHasRequiredRole(user.roleSet, next.data.allowedRoles);

        } else { // если пользователь неавторизован - отправить его на главную стр.
          this.router.navigateByUrl('login'); // корень сайта (главная страница)
